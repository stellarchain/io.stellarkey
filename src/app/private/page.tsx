...
export default function PrivatePaymentsPage() {
  return (
    ...
    >
        ...
        <p>StellarKey {APPLICATION_VERSION} uses Protocol V2 and one live, hash-pinned development pool on Testnet, with XLM at registry index 0 and USDC at index 1. The replacement pool started from fresh state on September 13, 2026. There is no backward-compatible state migration from the retired V1 pool: old notes, addresses and exposed proofs cannot be reused under the new deployment.</p>
        ...
        <p>Your recovery phrase rederives deployment-bound viewing keys. On a new device the wallet scans the canonical archive, authenticates the transcript and tree, decrypts owned notes and checks nullifiers against the chain. This depends on access to the matching deployment and its records; it does not migrate retired V1 notes into V2.</p>
