  ...
  return (
    ...
    >
      ...
      <g fontFamily={monoStack}>
        ...
        <text x="48" y="305" fontSize="11" fill={ink.dim}>· the transaction source and fee</text>
...
function SendPipelineDiagram() {
  const steps = [
    {
      n: "01 · choose",
      a: "picks notes to spend · two in, three out",
  ...
  return (
    ...
    >
      <title id="pipeline-title">
        The send pipeline in four steps: the wallet chooses up to two notes to
        spend and creates three outputs, an isolated worker builds a Groth16 proof over BN254
...
export default function PrivatePaymentsPage() {
  return (
    ...
    >
        ...
        <p>StellarKey {APPLICATION_VERSION} is the current application release, continuing the stable 1.0.0 application baseline. That application version is separate from Private Payments Protocol V2 and its unaudited Testnet-only status. One live, hash-pinned development pool on Testnet has XLM at registry index 0 and USDC at index 1. The pool started from fresh state on September 13, 2026. Only current deployment-bound notes, addresses, proofs and encrypted backups are supported; unsupported records are rejected without being rewritten.</p>
        ...
        <p>The bar for any non-development release is written down and enforced in code: a proving key verified against its pinned phase-one transcript, reproducible artifacts, a completed trusted-setup ceremony, independent contract and circuit review, immutable deployment evidence, and recovery drills, all tied to the same hashes. The current development key passes circuit and transcript compatibility checks but has no public ceremony or independent audit evidence. The published XLM/USDC pool is therefore a Testnet development deployment only, and Mainnet stays refused.</p></section>
        ...
        <details><summary>What does the fee account reveal?</summary><p>Every private send is submitted by your public Stellar account. The source account, fee payer, fee and timing remain public even when the internal transfer details are encrypted. Choosing another software account in the wallet to pay the fee does not hide either account.</p></details>
