...
const sections = [
  ...
  { id: "privacy-private-balance", label: "Private Payments status" },
...
export default function PrivacyPage() {
  return (
    ...
    >
        ...
        <section id="privacy-private-balance"><h2><DocShield />Private Payments status</h2><p>Production-hosted builds contain the exact hash-pinned replacement artifacts and one current XLM/USDC pool is published on Testnet; Mainnet independently refuses Private Payments. Its single-party proving key passes compatibility verification, but anyone retaining the setup secret could forge proofs. A multi-party ceremony, independent review, and new production-grade deployment are required before real-value use. The <a href={PUBLIC_ROUTES.private}>Private Payments explainer</a> documents the candidate and its privacy boundary.</p><p>The submitting Stellar account, timing, pool activity, proof, three commitments, nullifiers, and ciphertext remain public; deposits and withdrawals also reveal the asset, amount, and public endpoint. Internal transfers do not publish their asset. Direct mode uses the user&apos;s account; optional relay mode uses a selected peer discovered through public Nostr services and never silently falls back. Reusing one private address still correlates whole actions through its clear diversifier. Selected RPC and Nostr operators can observe the user&apos;s IP address, requested deployment, timing, and access pattern; the selected helper also learns the asset, fee, proof, and exact transaction.</p></section>
