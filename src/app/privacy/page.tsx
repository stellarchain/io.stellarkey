...
export default function PrivacyPage() {
  return (
    ...
    >
        ...
        <section id="privacy-private-balance"><h2><DocShield />Private Payments status</h2><p>Private Payments is currently a development-only testnet prototype and is unavailable in production builds. Its single-party development proving key failed verification against the repository&apos;s pinned Powers-of-Tau transcript, so the manifest is quarantined until a new multi-party ceremony, independent review, and fresh deployment bind to the same verified artifacts. The <a href={PUBLIC_ROUTES.private}>Private Payments explainer</a> documents the intended design and its privacy boundary; it is not an invitation to use the old pool.</p><p>In that design, the fee-paying Stellar account, timing, shared-deployment activity, proof, commitments, nullifiers, and ciphertext remain public; deposits and withdrawals also reveal the amount and public endpoint. A selected RPC can observe the user&apos;s IP address, requested deployment and ledgers, timing, simulations, and submissions.</p></section>
