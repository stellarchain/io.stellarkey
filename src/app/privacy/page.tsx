...
export default function PrivacyPage() {
  return (
    ...
    >
        ...
        <section id="privacy-private-balance"><h2><DocShield />Private Payments status</h2><p>Production-hosted builds permit the exact hash-pinned development fixture on Testnet only; Mainnet independently refuses Private Payments. Its single-party proving key passes compatibility verification, but anyone retaining the setup secret could forge Testnet proofs and take Testnet pool funds. A multi-party ceremony, independent review, and fresh deployment are required before real-value use. The <a href={PUBLIC_ROUTES.private}>Private Payments explainer</a> documents the preview and its privacy boundary.</p><p>The fee-paying Stellar account, timing, shared-deployment activity, proof, commitments, nullifiers, ciphertext, and current four-byte recipient-envelope diversifier remain public; deposits and withdrawals also reveal the amount and public endpoint. Reusing one private address is therefore correlatable. A selected RPC can observe the user&apos;s IP address, requested deployment and ledgers, timing, simulations, and submissions.</p></section>
