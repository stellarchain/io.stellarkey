...
export default function PrivacyPage() {
  return (
    ...
    >
        ...
        <section id="privacy-private-balance"><h2><DocShield />Private Payments status</h2><p>Private Payments shipped in release 1.3.0 as an explicitly unaudited, testnet-only preview. The production-hosted wallet may load it only while connected to Stellar Testnet and after you opt in for the current session; Mainnet rejects it. Its proving key comes from a single-party development setup with no recorded independent audit or multi-contributor ceremony, so the preview must not hold funds of value. A private transfer is designed to hide its recipient, amount, encrypted memo, and spent-note selection. Everything the feature owns stays local: owned balances, decrypted activity, recent private recipients, recovery progress, and pending actions are encrypted, deployment-bound records in this browser profile, and scanning for incoming payments happens locally by reading the public record with a locally derived viewing key. There is no application backend, relayer, or hosted indexer, and seed-only recovery reads the canonical encrypted archive directly from Stellar RPC. The <a href={PUBLIC_ROUTES.private}>Private Payments explainer</a> describes the full boundary.</p><p>The fee-paying Stellar account, timing, shared-deployment activity, proof, commitments, nullifiers, and ciphertext remain public; deposits and withdrawals also reveal the amount and public endpoint. The selected RPC can observe the user&apos;s IP address, requested deployment and ledgers, timing, simulations, and submissions. These limits apply on the testnet preview today and will still apply if a release passes its deployment and review gates.</p></section>
