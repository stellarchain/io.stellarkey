...
export default function PrivacyPage() {
  return (
    ...
    >
        ...
        <section id="privacy-private-balance"><h2><DocShield />Private Payments status</h2><p>Production-hosted builds contain the exact hash-pinned Protocol V2 artifacts and one current XLM/USDC pool is published on Testnet; Mainnet independently refuses Private Payments. Its single-party proving key passes compatibility verification, but anyone retaining the setup secret could forge proofs. A multi-party ceremony, independent review, and new production-grade deployment are required before real-value use. The <a href={PUBLIC_ROUTES.private}>Private Payments explainer</a> documents the implementation and its privacy boundary.</p><p>The submitting Stellar account, timing, pool activity, proof, output commitments, nullifiers, and ciphertext remain public; deposits and withdrawals also reveal the asset, amount, and public endpoint. Internal transfers do not publish their asset. Payments submit directly from the user&apos;s public Stellar account. Reusing one private address still correlates whole actions through its clear diversifier. Selected RPC operators can observe the user&apos;s IP address, requested deployment, timing, access pattern, proof, and exact transaction.</p></section>
