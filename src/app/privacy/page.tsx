...
export default function PrivacyPage() {
  return (
    ...
    >
        ...
        <section id="privacy-private-balance"><h2><DocShield />Private Payments status</h2><p>Production-hosted builds contain the exact hash-pinned replacement artifacts, but no active pool is published until fresh Testnet deployment evidence matches them; Mainnet independently refuses Private Payments. Its single-party proving key passes compatibility verification, but anyone retaining the setup secret could forge proofs. A multi-party ceremony, independent review, and fresh deployment are required before real-value use. The <a href={PUBLIC_ROUTES.private}>Private Payments explainer</a> documents the candidate and its privacy boundary.</p><p>For any future pool, the fee-paying Stellar account, timing, deployment activity, proof, commitments, nullifiers, and ciphertext remain public; deposits and withdrawals also reveal the amount and public endpoint. Reusing one private address is therefore correlatable. A selected RPC can observe the user&apos;s IP address, requested deployment and ledgers, timing, simulations, and submissions.</p></section>
