...
const sections = [
  ...
  { id: "security-private-balance", label: "Private Balance status" },
...
export default function SecurityPage() {
  return (
    ...
    >
        ...
        <section id="security-private-balance"><h2><DocLock />Private Balance status</h2><p>Private Balance remains development-only and is not mounted in the production wallet. Activation requires reproducible artifacts, a completed ceremony, independent contract and circuit review, immutable deployment evidence, recovery drills, and browser and physical-device release gates tied to the same hashes.</p><p>The design derives deployment-bound keys only inside an unlocked supported software-account callback, isolates proof work, encrypts owned notes and pending actions locally, reviews exact Soroban operations and restoration keys before signing, and reconciles against a canonical on-chain archive. It has no application backend. These controls do not hide the public fee payer, timing, pool activity, proof, commitments, nullifiers, or ciphertext, and deposits and withdrawals reveal their public endpoint and amount. Browser, device, origin, artifact, RPC, and correlation risks remain.</p></section>
