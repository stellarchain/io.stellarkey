...
export default function SecurityPage() {
  return (
    ...
    >
        ...
        <section id="security-private-balance"><h2><DocLock />Private Payments status</h2><p>Private Payments is currently a development-only testnet prototype and is unavailable in production builds. The checked-in single-party development key fails verification against the pinned Powers-of-Tau transcript. The manifest records that failure, and every non-development status now requires a verified key, non-zero ceremony transcript, independent audit reports, and deployment transactions. A new ceremony and deployment—not a status change—are required before another preview. <a href={PUBLIC_ROUTES.private}>Private payments, explained</a> covers the intended design.</p><p>The prototype derives deployment-bound keys only inside an unlocked supported software-account callback, isolates proof work in a worker, encrypts owned records and pending actions locally, reviews exact contract operations and restoration keys before signing, and reconciles against a canonical on-chain archive. These controls do not cure untrusted proving material and do not hide the public fee payer, timing, shared-deployment activity, proof, commitments, nullifiers, or ciphertext.</p></section>
