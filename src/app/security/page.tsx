...
export default function SecurityPage() {
  return (
    ...
    >
        ...
        <section id="security-private-balance"><h2><DocLock />Private Payments status</h2><p>Production-hosted builds publish one exact hash-pinned XLM/USDC development pool on Testnet; Mainnet independently refuses Private Payments. The checked-in evidence records its identity, locally selected Wasm hash, deployment and post-registry checkpoints, and read-back configuration; it does not include a deployment transaction hash or an independent on-chain executable-hash check. The single-party development key passes compatibility verification against the circuit and pinned Powers-of-Tau transcript, but that does not make its setup ceremony-secure. Anyone retaining the setup secret could forge proofs. A public multi-party ceremony, independent review, and complete browser payment evidence are required before any real-value or Mainnet promotion. <a href={PUBLIC_ROUTES.private}>Private payments, explained</a> covers the design and limits.</p><p>The implementation derives deployment-bound keys only inside an unlocked supported software-account callback, isolates proof work in a worker, encrypts owned records and pending actions locally, reviews exact contract operations and restoration keys before signing, and reconciles against a canonical on-chain archive. Payments submit directly from the user&apos;s account through the selected Stellar RPC. These controls do not cure untrusted proving material and do not hide the submitting account, timing, shared-pool activity, proof, commitments, nullifiers, ciphertext, or network metadata.</p></section>

        <section id="security-passkeys"><h2><DocFingerprint />Passkeys and hardware wallets</h2><p>Passkey unlock</p><p>On a compatible HTTPS origin, WebAuthn PRF derives an origin-bound secret after the platform authenticator requires user verification. StellarKey uses that result to unwrap the existing local vault master key. It does not receive a biometric template, export the authenticator’s private key, replace Stellar signing authority, or turn the wallet into a passkey smart account. The local passkey wrapper is not included in backups, so retain the wallet password and recovery material.</p><p>Trezor</p><p>When Trezor is selected, StellarKey sends the supported transaction description through the official Trezor Connect flow and asks the device to sign. Confirm the address and every transaction detail on the hardware display; a compromised browser can still try to present an unsafe request. StellarKey never asks for the Trezor recovery phrase. Trezor Connect is a separately licensed production dependency, loaded lazily for the optional hardware UI, and its services remain an external trust boundary.</p><p>Release {APPLICATION_VERSION} uses <code>@trezor/connect-web@9.7.3</code>, the pinned version reviewed for this release. The installed production audit reports ten low-severity vulnerable packages associated with one <code>elliptic</code> advisory through Trezor&apos;s Bitcoin/UTXO and browser-crypto dependencies, with no high or critical findings after a reviewed TOML parser override. That override does not rewrite the nested SDK&apos;s prebundled browser parser or Trezor&apos;s remote popup core; the embedded parser remains vulnerable and the remote core&apos;s deployed dependency status has not been verified here. The elliptic advisory lists no patched version and concerns ECDSA signing conditions; it does not establish exposure of Stellar Ed25519 keys. High and critical production advisories remain release-blocking. Audit results do not replace Trezor redistribution authorization, registered-origin approval, or physical-device checks.</p></section>

        <section id="security-web-security"><h2><DocGlobe />Web and release security</h2><p>Production must be served over HTTPS. The generated host policy applies a build-specific Content Security Policy, blocks framing and MIME sniffing, restricts browser permissions, separates the origin, and permits cross-origin popups needed by Trezor. The app has no dynamic API routes, server sessions, analytics script, advertising script, or cloud database. Its service worker caches the versioned static application shell and verified public proving artifacts, never wallet records or network account responses.</p><p>The CSP keeps <code>script-src</code> hash-bound with no <code>unsafe-inline</code> or JavaScript <code>unsafe-eval</code>. It permits <code>wasm-unsafe-eval</code> only so the browser can compile the local, hash-verified WebAssembly used to create Private Payments proofs; this does not enable JavaScript string evaluation. Its <code>connect-src https: wss:</code> and image policy are deliberately broad because user-configurable Stellar endpoints and issuer-chosen metadata or logo hosts cannot be enumerated at build time. This is an outbound-connection privacy boundary, not permission for a remote response to execute as script.</p><p>Every production build exposes the full source commit and a machine-readable release manifest. Published releases are expected to include artifact checksums and a CycloneDX software bill of materials. Compare those records with the public source. A matching identifier improves traceability but cannot alone prove that the host, build environment, or device is safe.</p></section>
        ...
        <p>For release 1.0.2, the maintainer explicitly deferred physical iPhone/iPad, VoiceOver/NVDA, passkey and Trezor checks, including Trezor redistribution and registered-origin sign-off. These checks are unperformed or unconfirmed, not passed. Automated browser profiles do not replace them. This release exception does not grant third-party license rights or approve real-value Private Payments.</p>
        ...
        <tbody><tr><td className="g">Selected Horizon / RPC endpoint</td><td>Operational balances, fees, simulation, merchant monitoring, and transaction submission</td><td>Your IP, requested public accounts, and signed transactions. Never your secret key.</td></tr><tr><td className="g">SDF public Horizon</td><td>Wallet activity and historical account operations, regardless of the custom endpoint setting</td><td>Your IP, requested public account, cursor, and timing.</td></tr><tr><td className="g">CoinGecko</td><td>Optional market and fiat display prices</td><td>Your IP, timing, and requested asset identifiers.</td></tr><tr><td className="g">Issuer-chosen metadata / logo hosts</td><td>Optional stellar.toml details and asset artwork</td><td>Your IP and the asset being viewed. Loading an issuer logo can therefore disclose interest in that asset to the issuer or its image host.</td></tr><tr><td className="g">Friendbot</td><td>Only when requesting testnet funding</td><td>Your IP and testnet public address.</td></tr><tr><td className="g">Trezor Connect</td><td>Only after choosing a Trezor action</td><td>Connection metadata and the supported unsigned transaction presented for device approval.</td></tr><tr><td className="g">External explorer</td><td>Only after opening an explorer link</td><td>The public account or transaction identifier you chose.</td></tr></tbody></table></div>
