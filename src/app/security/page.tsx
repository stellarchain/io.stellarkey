...
export default function SecurityPage() {
  return (
    ...
    >
        ...
        <section id="security-private-balance"><h2><DocLock />Private Payments status</h2><p>Production-hosted builds intentionally permit the exact hash-pinned development fixture on Testnet; Mainnet independently refuses Private Payments. The single-party development key passes compatibility verification against the circuit and pinned Powers-of-Tau transcript, but that does not make its setup ceremony-secure. Anyone retaining the setup secret could forge Testnet proofs and take Testnet pool funds. A public multi-party ceremony, independent review, fresh deployment, and recovery evidence are required before any real-value or Mainnet promotion. <a href={PUBLIC_ROUTES.private}>Private payments, explained</a> covers the design and limits.</p><p>The preview derives deployment-bound keys only inside an unlocked supported software-account callback, isolates proof work in a worker, encrypts owned records and pending actions locally, reviews exact contract operations and restoration keys before signing, and reconciles against a canonical on-chain archive. These controls do not cure untrusted proving material and do not hide the public fee payer, timing, shared-deployment activity, proof, commitments, nullifiers, ciphertext, or the current recipient-envelope diversifier.</p></section>
