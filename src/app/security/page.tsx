...
export default function SecurityPage() {
  return (
    ...
    >
        ...
        <section id="security-private-balance"><h2><DocLock />Private Payments status</h2><p>Production-hosted builds publish exact hash-pinned XLM and USDC development pools on Testnet; Mainnet independently refuses Private Payments. The checked-in deployment evidence records the live pool identities, locally selected Wasm hash, post-deployment checkpoints, and read-back configuration; it does not include a deployment transaction hash or an independent on-chain executable-hash check. The live browser suite exercises the end-to-end StellarKey path. The single-party development key passes compatibility verification against the circuit and pinned Powers-of-Tau transcript, but that does not make its setup ceremony-secure. Anyone retaining the setup secret could forge proofs. A public multi-party ceremony and independent review are required before any real-value or Mainnet promotion. <a href={PUBLIC_ROUTES.private}>Private payments, explained</a> covers the design and limits.</p><p>The candidate derives deployment-bound keys only inside an unlocked supported software-account callback, isolates proof work in a worker, encrypts owned records and pending actions locally, reviews exact contract operations and restoration keys before signing, and reconciles against a canonical on-chain archive. These controls do not cure untrusted proving material and do not hide the public fee payer, timing, shared-deployment activity, proof, commitments, nullifiers, or ciphertext.</p></section>
