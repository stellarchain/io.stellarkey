...
const sections = [
  ...
  { id: "about-private-payments", label: "Private Payments (preview)" },
...
export default function AboutPage() {
  return (
    ...
    >
        ...
        <section id="about-merchant"><h2><DocFingerprint />Merchant mode</h2><p>Merchant mode turns one unlocked wallet and browser into a single-device point of sale. Staff, shifts, orders, invoices, counter codes, customer records, cash and external-card tenders, refunds, and reports remain encrypted locally. The same device can face the buyer as a PIN-locked customer display. Crypto charges are matched against Horizon while the app is active; foreground monitoring pauses when the browser or operating system suspends the app and reconciles after it becomes active again.</p><p>Merchant mode is an operational aid, not cloud accounting, guaranteed payment monitoring, tax filing, card processing, or a durable off-device archive. Export records regularly and reconcile them against the ledger and your other payment providers.</p></section>

        <section id="about-private-payments"><h2><DocShieldDots />Private Payments (preview)</h2><p>Release {APPLICATION_VERSION} includes Private Payments Protocol V2, an unaudited, Testnet-only feature inside the same wallet: a private balance beside your public one for configured XLM and USDC. The asset, amount, recipient and memo of an internal transfer stay encrypted, while money moving in or out, the submitting account, its Stellar fee, and timing stay public. Proofs are created on this device with no StellarKey backend, and payments submit directly through the selected Stellar RPC using your public account. Seed recovery requires the matching deployment and authenticated canonical archive; an encrypted backup preserves local metadata and pending-proof records that the phrase alone cannot recover. Privacy grows with more independent activity; it is context, not a guarantee. The <a href={PUBLIC_ROUTES.private}>Private Payments explainer</a> covers the design, the limits, and the ceremony, review and deployment evidence still required before Mainnet.</p><p>The brand tells the same story: one mark, three duties. The official Stellar symbol, always at its own line weight, sits inside a lock for the wallet you own, a shield for the payments you keep private, and a receipt for the sales you can prove.</p></section>
