...
import { IconAlert, IconCheck, IconGift, IconPlus } from "./icons";
...
export function ClaimableBalancesModal({
  ...
  const selectedIds = useMemo(
    () => claimableBalances
      ...
      .map((item) => item.id),
    [balances, claimableBalances, selected],
  );
  const missingTrustlines = claimableBalances.length - availableIds.length;
  ...
  return (
    <Modal open onClose={onClose} wide dismissable={!busy}>
      <ModalHeader
        ...
        subtitle="Select only the assets you recognize"
        onClose={onClose}
      />
      <div className="space-y-4 p-4 sm:p-6">
        <div className="flex items-start gap-2.5 rounded-2xl border border-[#FF9F0A]/25 bg-[#FF9F0A]/10 px-3.5 py-3 text-[12px] leading-relaxed text-neutral-300">
          ...
          <p>
            ...
            on the public ledger; claiming is the only action this wallet can sign.
          </p>
        </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.025]">
          {claimableBalances.map((item, index) => {
            const ready = hasClaimTrustline(item, balances);
            const checked = selected.has(item.id);
            return (
              <label
                key={item.id}
                className={`flex min-h-[72px] items-center gap-3 px-4 py-3 text-left ${
        ...
        {(pendingSubmission || pendingAirdropClaim) && !error && (
          <div
            role="status"
