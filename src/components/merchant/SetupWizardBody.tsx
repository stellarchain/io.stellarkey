...
/* ------------------------------------------------------------------ */
/* Reporting to the shell                                              */
/* ------------------------------------------------------------------ */

/** The step header the body asks its owning shell to show. */
export interface SetupWizardHeader {
  title: string;
  subtitle: string;
  onBack?: () => void;
}
...
/**
 * Reports embedded state (saving, the step header) to the shell that owns the
 * dialog. The callback is read through a ref so the owner may pass an inline
 * function without re-triggering the report; unmounting reports the idle
 * value so the shell never stays busy or on a stale step after the body left.
 */
function useReportToShell<T>(report: (value: T) => void, value: T, idle: T): void {
...
export interface SetupWizardBodyProps {
  onClose: () => void;
  onComplete?: () => void;
  /** The shell's `initialFocus` target: the shop name field on step one. */
  nameRef: RefObject<HTMLInputElement | null>;
  onBusyChange: (saving: boolean) => void;
  onHeaderChange: (header: SetupWizardHeader | null) => void;
}

/**
 * The wizard's steps and draft. It needs the merchant provider
 * (`useMerchantConfiguration`, `useMerchantStatus`), so it mounts only inside
 * the requested merchant runtime; the shell in `SetupWizard.tsx` owns the
 * dialog and shows the header and busy state this body reports. It mounts on
 * open and leaves after the exit, so every opening starts on step one.
 */
export function SetupWizardBody({
  onClose,
  onComplete,
  nameRef,
  onBusyChange,
  onHeaderChange,
}: SetupWizardBodyProps) {
  ...
  async function finish() {
    if (problem || saving) {
      setShowProblems(true);
      if (problem) triggerHaptic("error");
      return;
    }
    ...
    try {
      ...
      toast("Merchant Mode is ready", "success", { silent: true });
      onComplete?.();
      onClose();
    } catch (error) {
      triggerHaptic("error");
  ...
  // The shell's header carries the step and its Back control; saving blocks
  // dismissal there too.
  const header = useMemo<SetupWizardHeader>(
    ...
    [current.title, goBack, step],
  );
  useReportToShell<SetupWizardHeader | null>(onHeaderChange, header, null);
  useReportToShell(onBusyChange, saving, false);
  ...
  return (
    <>
          ...
          {STEPS.map((entry) => {
            ...
            return (
              <li key={entry.n} className="min-w-0 flex-1">
                ...
                />
                <span
                  className={`mt-1.5 flex items-center gap-1 truncate text-[10.5px] font-semibold ${
                    here ? "text-white" : done ? "text-[#30D158]" : "text-neutral-500"
                  }`}
                >
                  {done && <IconCheck size={10} aria-hidden="true" className="shrink-0" />}
      ...
      <ModalBody>
        ...
        {/* Back lives in the header; the foot carries the way forward, and on the
            first step the way out. */}
        <ModalFooter
          secondary={
            step === 1 ? (
            ...
            <div key={rate.id} className="flex items-center gap-2.5">
              ...
              <div className="relative w-[96px] shrink-0">
                <input
                  ...
                  enterKeyHint="next"
                  className={`input mono !pr-8 text-base sm:text-[14px] ${
      ...
      <Block icon={<IconTerminal size={15} />} tint="#5E5CE6" title="This device">
        ...
        <div className="mt-3.5">
          ...
          <input
            ...
            className="input mono mt-2.5 text-base sm:text-[13px]"
          />
          <Hint>
