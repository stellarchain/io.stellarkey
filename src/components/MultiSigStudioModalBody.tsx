...
/** Header text the studio reports to its shell for the current stage. */
export type MultiSigStudioHeader = { title: string; subtitle?: string; onBack?: () => void };

export interface MultiSigStudioModalBodyProps {
  onBusyChange: (busy: boolean) => void;
  onDirtyChange: (dirty: boolean) => void;
  onHeaderChange: (header: MultiSigStudioHeader | null) => void;
}

// Mounted by the MultiSigStudioModal shell for each opening and kept through
// its exit. The tab list lives here because its options follow signer data.
export function MultiSigStudioModalBody({
  onBusyChange,
  onDirtyChange,
  onHeaderChange,
}: MultiSigStudioModalBodyProps) {
  ...
  const accountLabel = activeAccount?.label ?? "this account";
  ...
  // Sub-flows report their stage through the one header: a leading back
  // control replaces footer Back buttons, and the title names the stage.
  const inConfigureReview = tab === "configure" && reviewing;
  const inApprovalReview = tab === "approvals" && review !== null && !outcome;
  const headerTitle = inConfigureReview
    ? "Review Configuration"
    : inApprovalReview
      ? "Review Transaction"
      : "Multi-Sig Studio";
  const headerSubtitle = inConfigureReview
    ? `One atomic transaction on ${networkLabel}`
    : inApprovalReview && review
      ? `${review.operations.length} operation${review.operations.length === 1 ? "" : "s"} · ${review.networkLabel}`
      : `Shared control for ${accountLabel} · ${networkLabel}`;
  const headerBack = inConfigureReview ? "configure" : inApprovalReview ? "approval" : null;
  // Unsaved signer edits ask before they are discarded.
  const configureDirty =
    tab === "configure" && !reviewing && (signerChanges.length > 0 || newKey.trim() !== "");
  ...
  useLayoutEffect(() => {
    onDirtyChange(configureDirty);
    return () => onDirtyChange(false);
  }, [configureDirty, onDirtyChange]);

  useLayoutEffect(() => {
    onHeaderChange({
      title: headerTitle,
      subtitle: headerSubtitle,
      onBack: headerBack === "configure"
        ? () => setReviewing(false)
        : headerBack === "approval"
          ? clearReview
          : undefined,
  ...
  }, [clearReview, headerBack, headerSubtitle, headerTitle, onHeaderChange]);
  ...
  return (
    <ModalBody>
      <Tabs<Tab>
        ...
        options={[
          { value: "overview", label: "Overview", disabled: busy },
          { value: "configure", label: "Configure", disabled: !info || busy },
          { value: "approvals", label: "Approvals", disabled: busy },
        ]}
      >
      ...
      {tab === "overview" && (
        <div className="mt-5 space-y-4">
          {infoLoading ? (
            <LoadingRegion label="Loading signer configuration" className="min-h-40" />
              ...
              <ModalFooter
                primary={
                  <Button
                    type="button"
                    loading={busy}
                    loadingLabel="Applying configuration"
                  ...
                  {cosigners.map((c, i) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="-mr-2 flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-neutral-500 transition-colors hover:bg-[#FF453A]/10 hover:text-[#FF453A]"
          ...
          {!review && !outcome && (
            <>
              ...
              <Button
                ...
                className="w-full"
                loading={busy}
                loadingLabel="Decoding envelope"
                disabled={!xdrInput.trim() || Boolean(approvalGuardMessage)}
              ...
              <ModalFooter
                primary={
                  <Button
                    type="button"
                    loading={busy}
                    loadingLabel="Signing transaction"
                    disabled={!networkConfirmed || !review.signable || reviewExpired}
                    onClick={() => void handleCosign()}
                  >
                    Sign as {accountLabel}
                  </Button>
                }
              />
            </>
          ...
          {outcome && !outcome.submission && (
            <div className="panel-inset space-y-3 p-4">
              ...
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/10">
                <div
                  className="h-full w-full origin-left rounded-full bg-[#0A84FF] transition-transform duration-[var(--motion-duration-progress)]"
                  style={{
                    transform: `scaleX(${Math.min(1, outcome.collectedWeight / Math.max(1, outcome.requiredWeight))})`,
                  }}
                />
              </div>
      ...
      <ConfirmModal
        open={disableConfirm}
        title="Disable multi-sig?"
        message={
          <>
            This removes every cosigner and restores single-signature control to this device.
            {existingCosigners.length > 0 && (
              <span className="mt-3 block space-y-2">
                {existingCosigners.map((signer) => (
                  <span
                    className="mono block break-all rounded-xl bg-black/20 px-3 py-2 text-[11px] leading-relaxed text-white"
                    key={signer.key}
                  >
