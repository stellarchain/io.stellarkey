...
test('legacy receive address migrates once through encrypted issuance while old receipts remain recoverable', async t => {
  const manifest = networkManifest();
  ...
  const initial = { ...createEmptyPrivateBalanceState('01'.repeat(32), 1), privateAddress: legacy.address };
  ...
  for (const address of [legacy.address, replacement.address]) {
    ...
    assert.equal(recovered?.value === note.value, true, 'the unchanged root must recover both legacy and replacement receipts');
  }
});
