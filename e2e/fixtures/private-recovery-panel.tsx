...
import { createPrivateRecoveryScenario, readPrivateRecoveryBalances, readPrivateRecoveryState, type PrepareMode, type SubmitMode } from './private-recovery-scenario';
...
import { PrivateRecoveryProviderFixture } from './private-recovery-provider-panel';

type Scenario = Awaited<ReturnType<typeof createPrivateRecoveryScenario>>;
type Balances = Awaited<ReturnType<typeof readPrivateRecoveryBalances>>;

export function PrivateRecoveryFixture() {
  ...
  if (provider) return <PrivateRecoveryProviderFixture />;
  return <PrivateBalanceRuntimeDataProvider value={{ ...initialPrivateBalanceRuntimeData,
    ...
    <main data-app-surface className="min-h-screen space-y-4 p-6">
      <h1 className="text-xl">Synthetic three-wallet direct payment recovery</h1>
      <Button disabled={!!scenario} onClick={() => setProvider(true)}>Test real recovery provider</Button>
      <p>Non-usable fixtures only. Proof generation and network responses are controlled.</p>
      ...
      <div><label htmlFor="synthetic-prepare">Synthetic preparation response</label>
        <select id="synthetic-prepare" value={prepareMode} disabled={busy} onChange={event => setPrepareMode(event.target.value as PrepareMode)}>
          {(['approve', 'proof-failure', 'consent-expired', 'rpc-reject', 'rpc-timeout'] as const).map(mode => <option key={mode}>{mode}</option>)}
        </select></div>
      ...
      <Button disabled={busy || !!scenario} onClick={() => { void run(async () => {
        const created = await createPrivateRecoveryScenario(development as PrivateBalanceManifest, driver(), { deposits: deposits.split(','), amount });
      ...
      <Button disabled={busy || !scenario?.review} onClick={submit}>Submit directly</Button>
      ...
      <Button disabled={busy} onClick={() => { void run(async () => {
        const stored = await readPrivateRecoveryBalances(development as PrivateBalanceManifest, driver());
        if (mounted.current) { setBalances(stored); setDurable(await readPrivateRecoveryState(development as PrivateBalanceManifest, driver())); }
        return 'restored';
      }); }}>Inspect persisted synthetic balances</Button>
      <p data-testid="private-recovery-status">{status}</p>
      <p data-testid="private-recovery-fresh">{fresh}</p>
      <p data-testid="private-recovery-shared">{scenario?.shared ?? 0}</p>
      <p data-testid="private-recovery-submissions">{scenario?.submissions ?? 0}</p>
      <p data-testid="private-recovery-sender-lookups">{scenario?.senderLookups ?? 0}</p>
      {balances ? Object.entries(balances).map(([name, value]) => <p key={name}>{name}: <span data-testid={`private-recovery-${name}`}>{value}</span></p>) : null}
