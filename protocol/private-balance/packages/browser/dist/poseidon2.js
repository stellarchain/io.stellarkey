import { BN254_FR_MODULUS, bigintTo32Bytes, bytesToBigint, bytesToField, } from './field.js';
import { sha256Bytes, utf8 } from './hash.js';
import { POSEIDON2_DIAGONAL, POSEIDON2_ROUND_CONSTANTS, } from './generated/poseidon2-parameters.js';
const RATE = 3;
const WIDTH = 4;
const FULL_ROUNDS_PER_SIDE = 4;
const PARTIAL_ROUNDS = 56;
const LENGTH_IV_FACTOR = 1n << 64n;
const PINNED_DOMAIN_FIELDS = new Map([
    ['SKSB_OWNER_V1', '0e87c9065d3f311d9f01a6beb1d6eefd2a04031eca1b3dc2cb39e3aeab3b2164'],
    ['SKSB_DIVERSIFIED_OWNER_V1', '2062e34540425ef36807b252f10c54d75d5a5208d8d4555cb1f0aa4a3bff6d7d'],
...
function permutation(input) {
    ...
    for (let round = 0; round < FULL_ROUNDS_PER_SIDE; round += 1) {
        state = externalMatrix(state.map((value, index) => sbox5(value + POSEIDON2_ROUND_CONSTANTS[round * WIDTH + index])));
    }
    ...
    for (let block = 0; block < blockCount; block += 1) {
        for (let rateIndex = 0; rateIndex < RATE; rateIndex += 1) {
            ...
            if (inputIndex < inputs.length)
                state[rateIndex] = mod(state[rateIndex] + inputs[inputIndex]);
        }
        state = permutation(state);
    }
    return state[0];
}
export function poseidonDomainField(domain) {
...
export function p2(domain, inputs) {
