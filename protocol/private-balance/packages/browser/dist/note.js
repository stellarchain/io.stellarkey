...
function validateNote(note) {
    ...
    if (note.reserved.some((byte) => byte !== 0))
        throw new Error('Reserved note bytes must be zero');
}
export function encodeNotePlaintext(note) {
    validateNote(note);
...
export function decodeNotePlaintext(bytes) {
    if (bytes.length !== NOTE_PLAINTEXT_BYTES)
        throw new Error('Invalid note plaintext length');
    let value = 0n;
