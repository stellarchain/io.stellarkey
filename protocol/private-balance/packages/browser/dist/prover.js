...
function fieldStrToBytes(s) {
    if (!/^(?:0|[1-9][0-9]*)$/.test(s))
        throw new Error('Invalid proof field spelling');
    let n = BigInt(s);
    if (n >= BN254_BASE_MODULUS)
        throw new Error('Proof coordinate is outside BN254 Fq');
...
export async function proveAction(circuitInputs, wasmPathOrBuffer, zkeyPathOrBuffer) {
    const { proof, publicSignals } = await snarkjs.groth16.fullProve(circuitInputs, wasmPathOrBuffer, zkeyPathOrBuffer);
...
export async function verifyProofLocally(vk, publicSignals, proof) {
