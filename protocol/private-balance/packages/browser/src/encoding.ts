...
function validateOutgoingPlaintext(plaintext: OutgoingPlaintext): void {
  ...
  if (dummy && plaintext.memoLength !== 0) throw new Error('Dummy outgoing memo must be empty');
  if (plaintext.reserved.some(byte => byte !== 0)) throw new Error('Outgoing reserved bytes must be zero');
}

export function encodeOutgoingPlaintext(plaintext: OutgoingPlaintext): Uint8Array {
...
export function decodeOutgoingPlaintext(bytes: Uint8Array): OutgoingPlaintext {
  ...
  const plaintext: OutgoingPlaintext = {
...
export function deriveOutgoingAad(
