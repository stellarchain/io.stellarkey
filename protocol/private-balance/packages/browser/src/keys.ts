...
function deriveNonzeroField(
  ...
  context: Uint8Array,
  startCounter = 0,
): { field: Uint8Array; counter: number } {
...
export function derivePrivacySessionRoot(
  rawStellarSeed: Uint8Array,
  protocolVersion: number,
  networkId: Uint8Array,
  realmId: Uint8Array,
  poolId: Uint8Array,
  ...
  if (rawStellarSeed.length !== 32) throw new Error('raw Stellar seed must be 32 bytes');
...
export function derivePrivateStorageKey(
  privacySessionRoot: Uint8Array,
  deploymentBindingHash: Uint8Array,
): Uint8Array {
  if (privacySessionRoot.length !== 64) throw new Error('privacy session root must be 64 bytes');
  if (deploymentBindingHash.length !== 32) throw new Error('deployment binding hash must be 32 bytes');
  const expanded = hmacSha512(
    privacySessionRoot,
    utf8(DOMAIN_STORAGE_KEY),
...
export async function deriveExpandedSpendingKey(
  privacySessionRoot: Uint8Array,
  protocolVersion: number,
  networkId: Uint8Array,
  realmId: Uint8Array,
...
): Promise<ExpandedSpendingKey> {
  if (privacySessionRoot.length !== 64) throw new Error('privacy session root must be 64 bytes');
  if (contextField.length !== 32) throw new Error('context field must be 32 bytes');
  const context = keyContext(
    protocolVersion,
    networkId,
    realmId,
  ...
  try {
    ...
    const outgoingViewingKey = hkdfExpand(
      prk,
...
export async function deriveKeysFromSeed(
  ...
  try {
    return await deriveExpandedSpendingKey(
      sessionRoot,
      protocolVersion,
      networkId,
      realmId,
...
export function toViewingKey(esk: ExpandedSpendingKey): FullViewingKey {
  return {
