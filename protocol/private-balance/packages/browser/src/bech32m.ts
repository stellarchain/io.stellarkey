...
function createChecksum(hrp: string, words: readonly number[]): number[] {
...
function convertBits(
  ...
  for (const value of values) {
    if (value < 0 || value >>> fromBits !== 0) throw new Error('Invalid Bech32m data value');
...
export function decodeBech32m(
  ...
  if (payload.length !== expectedPayloadBytes) throw new Error('Invalid Bech32m payload length');
  return payload;
}

export function groupBech32m(encoded: string): string {
