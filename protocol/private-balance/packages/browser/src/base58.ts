...
const BASE58_VALUES = new Map(
  Array.from(BASE58_ALPHABET, (character, index) => [character, BigInt(index)] as const),
);

export function encodeBase58(bytes: Uint8Array): string {
  if (!(bytes instanceof Uint8Array)) throw new Error('Base58 input must be bytes');
...
export function decodeBase58(encoded: string): Uint8Array {
  ...
  if (encodeBase58(bytes) !== encoded) throw new Error('Noncanonical Base58 spelling');
  return bytes;
}
