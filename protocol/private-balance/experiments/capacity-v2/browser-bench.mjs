...
import { fullCrossFixture } from './fixtures.mjs';

const root = import.meta.dirname;
const files = new Map([
  ['/snarkjs.js', [join(root, '../../../../node_modules/snarkjs/build/snarkjs.min.js'), 'text/javascript']],
  ['/action.wasm', [join(root, 'build/full/action_js/action.wasm'), 'application/wasm']],
  ['/action.zkey', [join(root, 'build/full/action_dev.zkey'), 'application/octet-stream']],
  ['/vk.json', [join(root, 'build/full/verification_key.json'), 'application/json']],
]);
// An explicit allowlist serves only prototype proving artifacts on loopback.
...
writeFileSync(join(root, 'build/browser.json'), JSON.stringify(results, null, 2) + '\n');
