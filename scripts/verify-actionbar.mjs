...
for (const mobile of [false, true]) {
  ...
  // hover the Swap circle for a state check
  await page.hover("div.max-w-\\[360px\\] > button:has-text('Swap')");
  await page.waitForTimeout(250);
  await page.screenshot({ path: `/tmp/shots/ios-actions-${mobile ? "mobile" : "desktop"}.png` });
  await context.close();
}
await browser.close();
console.log("done");
