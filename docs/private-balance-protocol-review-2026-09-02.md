...
|---|---|---|---|
| PKCS#8 X25519 import | Accept | Byte-identical shared secret; warm native median improved 77.19% in the final three-trial run | Native WebCrypto imports the raw scalar through RFC 8410 PKCS#8; partial or transient native failure retries the portable path |
| Recipient scan path | Accept | RFC 9180-compatible WebCrypto measured 4.57x the old path; the configured batch 8 measured a 2.323x paired median against adjacent sequential controls over nine trials | View-tag-first rejection and bounded ordered scanning shipped; the claimed 11x did not reproduce, and the conservative cap is fixed rather than chosen from a noisy local winner |
...
| Verifier and pool contract reductions | Accept | Transfer verification: 39,614,514 → 29,287,953 instructions (−26.07%); like-for-like pool Wasm: 93,504 → 66,377 bytes raw (−29.01%) and 80,245 → 57,042 optimized (−28.92%) | Uses one BN254 MSM, derives signals once, precomputes the asset field, uses fixed-width field arithmetic, and removes dead paging configuration |
